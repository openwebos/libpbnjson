...
#include "../c_api/JSXMLConverter.h"
...
static void generate(jvalue_ref jval, JStreamRef s)
{
	if (jis_object(jval)) {
		s->o_begin(s);
		typedef QMap<jvalue_ref, jvalue_ref> JObject;
		JObject sorted;
...
void TestSAX::testParser()
{
	...
	try {
		...
		QVERIFY2(jsax_parse(&myCallbacks, j_str_to_buffer(input.constData(), input.size()), NULL, &userData), input);
...
static QString simplify(QString formattedJSON)
{
	...
	if (!jsax_parse(&myCallbacks, j_str_to_buffer(input.constData(), input.size()), NULL, &userData)) {
...
void TestSAX::testGenerator_data()
{
	...
	QTest::newRow("sanity object check") <<
		jobject_create() << "{}" << jsxml("<jsxml:object obj:length='0' />");

	QTest::newRow("sanity array check") <<
		jarray_create(NULL) << "[]" << jsxml("<jsxml:array arr:length='0' />");
	...
	QTest::newRow("complex json check") <<
		...
		// array1 -> object1 -> numbers
		simplify("{																								\
			\"array1\": [																						\
				\"testing1\",\"test2\",42323,2354235235.23523892389e23343,[true,false,null],null,\"\",\"\"		\
			],																									\
			\"object1\":{																						\
				\"test2\":\"test1\",																			\
				\"test1\":\"test2\"																				\
...
static bool same(QDomDocument actual, QDomDocument expected)
{
	return true;
}
...
QTEST_APPLESS_MAIN(pjson::testcxx::TestSAX)
